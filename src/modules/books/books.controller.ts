...
const addBook = async (req: Request, res: Response) => {
    try {
        ...
        if (!book) {
            res.status(500).json(ApiError(500, "Failed to create book", req, ["An error occurred while creating the book"]));
            return
        }
        res.status(201).json(ApiResponse(201, book, "Book created successfully"));
        return
    } catch (error: any) {
        res.status(500).json(ApiError(500, "Server error", req, [error.message || "An error occurred while creating the book"]));
        return
    }
};
...
const updateBook = async (req: Request, res: Response) => {
    try {
        ...
        if (!book) {
            res.status(500).json(ApiError(500, "Failed to update book", req, ["An error occurred while updating the book"]));
            return
        }
        res.status(200).json(ApiResponse(200, book, "Book updated successfully"));
        return
    } catch (error: any) {
        res.status(500).json(ApiError(500, "Server error", req, [error.message || "An error occurred while updating the book"]));
        return
    }
};
const getBooks = async (req: Request, res: Response) => {
    try {
        ...
        res.status(500).json(ApiError(500, "Server error", req, ["An error occurred while fetching books"]));
        return
    }
};
...
const getSingleBook = async (_req: Request, res: Response) => {
    ...
    try {
        ...
        if (!id) {
            res.status(400).json(ApiError(400, "Invalid Book ID", _req, ["Book ID is not provided"]));
            return
        }
        ...
        if (!book) {
            res.status(404).json(ApiError(404, "Book not found", _req, ["No book found with the provided ID"]));
            return
        }
        ...
        res.status(500).json(ApiError(500, "Server error", _req, ["An error occurred while retrieving the book"]));
        return
    }
}
...
const deleteBook = async (_req: Request, res: Response) => {
    try {
        ...
        res.status(500).json(ApiError(500, "Server error", _req, [error.message || "An error occurred while deleting the book"]));
        return
    }
};
