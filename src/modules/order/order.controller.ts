...
import { sendOrderConfirmationEmail } from "src/utils/sendMail";
import { v4 as uuidv4 } from 'uuid';
import { generateOrderEmailHTML, generateOrderTableData } from "src/constants/mailTemplate";
import { Payments } from "src/model/payment.model";
import { OrderStatus, PaymentStatus } from "src/constants";
...
const placeOrder = async (req: Request, res: Response) => {
    try {
        ...
        if (customerCart.length === 0) {
            res.status(400).json(ApiError(400, "Cart is empty", req, ["Add items to cart before placing an order"]));
            return;
        }

        const outOfStockItems = customerCart.filter(item =>
        ...
        res.status(201).json(ApiResponse(201, created, "Order placed successfully"));
    } catch (err) {
        if (err instanceof Error) {
            res.status(500).json(ApiError(500, "Internal Server Error", req, [err.message]));
        } else {
            res.status(500).json(ApiError(500, "Internal Server Error", req, ["An unexpected error occurred"]));
        }
    }
};
...
const verifyPayment = async (req: Request, res: Response) => {
    ...
    if (!razorpay_order_id || !razorpay_payment_id || !razorpay_signature) {
        res.status(400).json(ApiError(400, "Bad Request", req, ["invalid payload"]));
        return
    }
    try {
        ...
        if (!isValid) {
            res.status(400).json(ApiError(400, "Bad Request", req, ["Invalid payment details"]));
            return
        }
        ...
        if (!order) {
            res.status(404).json(ApiError(404, "Order not found", req, ["Invalid order id"]));
            return;
        }
        ...
        await db.transaction(async (tx) => {
            ...
            for (const it of items) {
                ...
                await tx.update(Books).set({ stock: book.stock - it.quantity }).where(eq(Books.id, it.bookId));
            }
...
const listMyOrders = async (req: Request, res: Response) => {
    try {
        ...
        res
            .status(200)
            .json(
                ApiResponse(200, { items: orders, page, pageSize, total: Number(count) }, "Orders fetched")
            );
    } catch (err) {
        res.status(500).json(ApiError(500, "Internal Server Error", req, err instanceof Error ? [err.message] : ["An unexpected error occurred"]));
        return;
    }
};
...
const getOrderById = async (req: Request, res: Response) => {
    try {
        ...
        if (!orderId) {
            res.status(400).json(ApiError(400, "Bad Request", req, ["Order ID is required"]));
            return;
        }
        ...
        res.status(200)
            .json(
                ApiResponse(200, { order, items }, "Order details fetched")
            );
        return
    } catch (err) {
        res.status(500).json(ApiError(500, "Internal Server Error", req, err instanceof Error ? [err.message] : ["An unexpected error occurred"]));
...
const cancelOrder = async (req: Request, res: Response) => {
    try {
        ...
        res.status(200)
            .json(
                ApiResponse(200, updatedOrder, message)
            );
        return
    } catch (err) {
        res.status(500).json(ApiError(500, "Internal Server Error", req, err instanceof Error ? [err.message] : ["An unexpected error occurred"]));
