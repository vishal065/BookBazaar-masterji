...
const addToCart = async (req: Request, res: Response) => {
  try {
    ...
    if (!userId) {
      res
        .status(400)
        .json(ApiError(400, "unauthorized", req, ["User ID is not provided"]));
      return;
    }
    ...
    if (!book) {
      res
        .status(404)
        .json(
          ApiError(404, "Book not found", req, [
    ...
    if (cart) {
      res
        .status(409)
        .json(
          ApiError(409, "Book already in cart", req, ["Book already in cart"]),
        );
      return;
    }
    ...
    if (!cartItem) {
      res
        .status(500)
        .json(
          ApiError(500, "Failed to add book to cart", req, [
            "An error occurred while adding book to cart",
          ]),
        );
      return;
    }
  ...
  try {
    ...
    if (!id) {
      res
        .status(400)
        .json(
          ApiError(400, "Invalid cart item ID", req, [
            "Cart item ID is not provided",
          ]),
        );
      return;
    }
    ...
    if (!cartItem) {
      res
        .status(404)
        .json(
          ApiError(404, "Cart item not found", req, [
            "No cart item found with the provided ID",
          ]),
        );
      return;
    }
...
const getCartItems = async (req: Request, res: Response) => {
  try {
    ...
    if (!userId) {
      res
        .status(401)
        .json(ApiError(401, "unauthorized", req, ["User ID is not provided"]));
      return;
    }
  ...
  try {
    ...
    if (quantity < 0 || !quantity) {
      res
        .status(400)
        .json(
          ApiError(400, "Invalid quantity", req, [
    ...
    if (quantity === 0) {
      ...
      res
        .status(200)
        .json(ApiResponse(200, cartItem, "Cart removed successfully"));
      return;
    }
